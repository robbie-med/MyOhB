...
// ═══════════════════════════════════════════════════════
const CONTACTS = [
  { id: 'familydoc', label: 'Family Doctor', hasName: true, hasPhone: true },
  { id: 'ob', label: 'OB / Midwife', hasName: true, hasPhone: true },
  { id: 'peds', label: 'Pediatrician', hasName: true, hasPhone: true },
  { id: 'mfm', label: 'MFM Specialist', hasName: true, hasPhone: true },
  { id: 'lactation', label: 'Lactation Consultant', hasName: true, hasPhone: true },
  { id: 'ld', label: 'Hospital / L&D Triage', hasName: false, hasPhone: true },
  { id: 'pharmacy', label: 'Pharmacy', hasName: true, hasPhone: true },
];

const DETAILS = [
  { id: 'edd', label: 'Due Date (EDD)', placeholder: 'e.g. July 15, 2026', type: 'text' },
  { id: 'bloodtype', label: 'Blood Type & Rh Factor', placeholder: 'e.g. O positive', type: 'text' },
  { id: 'gbs', label: 'GBS Status', placeholder: 'Positive / Negative / Not yet tested', type: 'text' },
  { id: 'allergies', label: 'Medication Allergies', placeholder: 'e.g. Penicillin', type: 'text' },
  { id: 'insurance', label: 'Insurance / Member ID', placeholder: 'For quick reference at hospital', type: 'text' },
];
...
const SEARCH_INDEX = [];
const SECTION_META = {
  pregnancy: { label: 'Prenatal', color: '' },
  labor:     { label: 'Labor & Delivery', color: 'gold' },
  recovery:  { label: 'Recovery', color: 'rose' },
  baby:      { label: 'Newborn', color: 'navy' },
  faq:       { label: 'FAQ', color: 'plum' },
};
  ...
  items.forEach(item => {
    ...
    if (isFaq) {
      const badge = item.type === 'myth'
        ? '<span class="faq-badge badge-myth">Myth</span>'
        : '<span class="faq-badge badge-faq">FAQ</span>';
      card.innerHTML = `
...
const TOOL_PAGES = ['tools','tool-kick','tool-contractions','tool-feeding',
  ...
  'tool-mood','tool-birthplan','tool-appts'];
...
function doSearch() {
  ...
  const results = SEARCH_INDEX.filter(item => {
    return terms.every(t => item.titleLower.includes(t) || item.bodyLower.includes(t));
  }).slice(0, 20);
  ...
  if (results.length === 0) {
    searchResultsList.innerHTML = `<div class="no-results"><strong>No results found</strong>Try different keywords — like "epidural", "cord care", or "bleeding"</div>`;
    return;
  }
  ...
  results.forEach(item => {
    ...
    let snippet = item.body;
    const idx = item.bodyLower.indexOf(firstTerm);
    if (idx > -1) {
      const start = Math.max(0, idx - 60);
      const end = Math.min(item.body.length, idx + firstTerm.length + 100);
    ...
    div.innerHTML = `
      <div class="sri-section ${meta.color}">${meta.label}</div>
      <div class="sri-title">${item.title}</div>
      <div class="sri-snippet">${highlighted}</div>`;
...
function renderMyInfo() {
  ...
  CONTACTS.forEach(c => {
    ...
    row.innerHTML = `
      ...
      <div class="contact-fields">
        ${c.hasName ? `<input class="contact-input" id="ci_${c.id}_name" type="text" placeholder="Name" value="${escHtml(nameVal)}">` : ''}
        <div class="contact-field-wrap">
          <input class="contact-input" id="ci_${c.id}_phone" type="tel" placeholder="Phone number" value="${escHtml(phoneVal)}" inputmode="tel">
          <button class="action-btn call-btn" onclick="callContact('${c.id}')" title="Call" ${!phoneVal ? 'disabled' : ''}>
          ...
          <button class="action-btn text-btn" onclick="textContact('${c.id}')" title="Text" ${!phoneVal ? 'disabled' : ''}>
  ...
  DETAILS.forEach(d => {
    ...
    row.innerHTML = `
      <div class="contact-row-label">${d.label}</div>
      <input class="contact-input" id="di_${d.id}" type="${d.type}" placeholder="${d.placeholder}" value="${escHtml(val)}">`;
...
function saveInfo() {
  ...
  showToast('Contacts saved to your phone');
  setTimeout(() => {
...
window.addEventListener('appinstalled', () => {
  document.getElementById('install-btn').classList.remove('visible');
  showToast('App installed! Open from your home screen anytime');
});
      ...
      reg.addEventListener('updatefound', () => {
        ...
        newWorker.addEventListener('statechange', () => {
          if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
            showToast('App updated — reload for latest version', 4000);
          }
        });
      });
    }).catch(console.error);
